...
**CLI scan** (optional — the standalone `npx impeccable detect` file-scanner ships with the upstream `impeccable` package, which this template does not bundle):
```bash
npx impeccable detect --json [--fast] [target]
```

- If you have the upstream CLI installed: pass HTML/JSX/TSX/Vue/Svelte files or directories as `[target]` (anything with markup); do not pass CSS-only files. For URLs, skip the CLI scan (it requires a headless browser) and use browser visualization instead. For large directories (200+ scannable files), use `--fast` (regex-only, skips jsdom); for 500+ files, narrow scope or ask the user. Exit code 0 = clean, 2 = findings.
- If you do not have it, skip straight to the **browser visualization** below — it runs the same bundled deterministic detector via the live server and is the primary path in this template.
