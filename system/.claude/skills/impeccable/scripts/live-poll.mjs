 ...
 *
 * Usage:
 *   node .claude/skills/impeccable/scripts/live-poll.mjs                         # Block until browser event, print JSON
 *   node .claude/skills/impeccable/scripts/live-poll.mjs --timeout=600000        # Custom timeout (ms); default is long-poll friendly
 *   node .claude/skills/impeccable/scripts/live-poll.mjs --reply <id> done       # Reply "done" to event <id>
 *   node .claude/skills/impeccable/scripts/live-poll.mjs --reply <id> error "msg" # Reply with error
 */
...
function readServerInfo() {
  ...
  if (!record) {
    console.error('No running live server found. Start one with: node .claude/skills/impeccable/scripts/live.mjs');
    process.exit(1);
  }
  return record.info;
}
...
Options:
  ...
  // Reply mode: node .claude/skills/impeccable/scripts/live-poll.mjs --reply <id> <status> [--file path] [message]
  ...
  if (replyIdx !== -1) {
    ...
    if (!id) {
      console.error('Usage: node .claude/skills/impeccable/scripts/live-poll.mjs --reply <id> <status> [--file path] [message]');
      process.exit(1);
    }
    ...
    try {
      ...
      if (err.cause?.code === 'ECONNREFUSED') {
        console.error('Live server not running. Start one with: node .claude/skills/impeccable/scripts/live.mjs');
      } else {
  ...
  try {
    while (true) {
      ...
      if (res.status === 401) {
        ...
        console.error('Try restarting: node .claude/skills/impeccable/scripts/live-server.mjs stop && node .claude/skills/impeccable/scripts/live.mjs');
        process.exit(1);
      }
